...
class Serial_IMU(UartAXI):
    def __init__(self, overlay, direction = 0):
        ...
        super().__init__(address, overlay)
    ...
    def get_Imu(self):
        ...
        if sign:
            ...
            ACC[0]=np.int32(Re_buf[4]<<8|Re_buf[5])   #合成数据，去掉小数点后2位
            ACC[1]=np.int32(Re_buf[6]<<8|Re_buf[7])
            ACC[2]=np.int32(Re_buf[8]<<8|Re_buf[9])

            gyro[0]=np.int32(Re_buf[10]<<8|Re_buf[11])   #合成数据，去掉小数点后2位
            gyro[1]=np.int32(Re_buf[12]<<8|Re_buf[13])
            gyro[2]=np.int32(Re_buf[14]<<8|Re_buf[15])
